...
class Notification {
  ...
  addNotification() {
    for (let deviceId in this.nDevices) {
      ...
      this.nPayload.payload.devices.notifications[deviceId] = this.nDevices[deviceId];
    }
  }
...
function sendNotification(agentUserId, deviceId, stateObject, options) {
  ...
  if (options && options.reportState) builderObj.addState(deviceId, stateObject);
  if (options && options.notify) builderObj.addNotification(deviceId, stateObject);

  const n = builderObj.build();

  var jsonBody = n.toJSON();
  console.log("sendNotification: notification object", JSON.stringify(jsonBody, null, 2));
...
io.on('connection', function (socket) {
  ...
  socket.on(DEVICE_INTERFACE.COMMANDS.CHIME, function() {
    sendNotification("98709sad0f9j09a", "IO2018One", {
      "ObjectDetected": {
        "objects": {
          "NAMED": [
            "mail"
